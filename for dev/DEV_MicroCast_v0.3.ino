...
int commentSel = 0; // выбранный комментарий
int commentScrollOffset = 0; // scroll для комментариев
    ...
    if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
      ...
      // Добавление символов в позицию курсора
        ...
        for (char c : status.word) {
          // Теперь , и / разрешены! Фильтруем только служебные
          if (c != 0x1B && c != '`' && c != ';' && c != '.' &&
...
void drawCommentsView() {
  ...
  if (commentCount == 0) {
    ...
    for (int i = 0; i < commentsPerScreen; i++) {
      ...
      // Автор комментария (голубой)
      d.setCursor(3, boxY);
      d.setTextColor(0x07FF, 0x0000);
      d.print(c.author);
      
      // Время (справа от автора, серый)
      if (c.created_at.length() >= 16) {
        String time = c.created_at.substring(11, 16); // HH:MM
        d.setCursor(90, boxY);
        d.setTextColor(0x8410, 0x0000);
        d.print(time);
      }
      
      // Текст комментария (белый, макс 1 строка)
      d.setCursor(3, boxY + 8);
      d.setTextColor(0xFFFF, 0x0000);
      String commentText = c.text;
      if (commentText.length() > 38) {
        d.print(commentText.substring(0, 35) + "...");
      } else {
        d.print(commentText);
      }
    }
...
void loop(){
  ...
  if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()){
    ...
    // Навигация: вверх/вниз
      ...
      if (viewingComments) {
        // Scroll комментариев вверх
        if (commentScrollOffset > 0) {
          commentScrollOffset--;
          drawCommentsView();
        }
      ...
      if (viewingComments) {
        // Scroll комментариев вниз
        int commentsPerScreen = 3;
        if (commentScrollOffset < (int)commentCount - commentsPerScreen) {
          commentScrollOffset++;
          drawCommentsView();
        }
      } else if (isInSectionSelector) {
